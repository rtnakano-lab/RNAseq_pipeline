#!/bin/bash
# bash script to add new samples to already-processed samples.
# preprocessing of reads by fastp, mapping to araport11 genome by HiSAT2, and read count by featureCount
...
# prerequisites:
# appropriate edits of 0-config_add.bash
# list of libraries *to add* in a tab-separted text file with a single-line header, containing the name of Libraries in the first column -> design_add.txt
# list of *ALL* libraries, including the additional ones, in a tab-separted text file with a single-line header, containing the name of Libraries in the first column -> design.txt
...
# bsub -J QC_map_count -q multicore40 -n 40 -R "rusage[mem=24000] span[hosts=1]" -M 64000 ./1-QC_map_count_add.bash ./0-config_add.bash
# -J: job name
...
do
	...
	echo ${lib} >> ${map_dir}/HiSat2-map_summary_all.txt
